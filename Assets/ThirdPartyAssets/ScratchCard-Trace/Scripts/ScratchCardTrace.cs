...
{
	public class ScratchCardTrace : MonoBehaviour
	{
		public enum Quality
		{
			Low = 4,
		...
		private ScratchCardRendererTrace cardRenderer;
		private ScratchCardInputTrace cardInput;
		private TriangleTrace triangle;
		...
		void Update()
		{
			...
			{
                if (!ABCManager.instance.ColorScratch.isPlaying&&ABCManager.instance.ColorScratch.enabled)
                {
					ABCManager.instance.ColorScratch.Play();
				}
				//print("mouseD");
				cardInput.Scratch();
			}
			...
			{
				if (ABCManager.instance.ColorScratch.isPlaying)
				{
		...
		{
			cardInput = new ScratchCardInputTrace(this);
			...
			cardRenderer = new ScratchCardRendererTrace(this);
		...
		private void InitTriangle()
		{
			...
			var uv2 = Vector2.one;
			triangle = new TriangleTrace(position0, position1, position2, uv0, uv1, uv2);
		}
