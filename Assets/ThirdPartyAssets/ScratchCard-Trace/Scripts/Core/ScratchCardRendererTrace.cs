...
namespace ScratchCardAsset.Core
{
	...
	/// </summary>
	public class ScratchCardRendererTrace
	{
		...
		public ScratchCardRendererTrace(ScratchCardTrace card)
		{
			scratchCard = card;
			localBounds = new Bounds(Vector2.one / 2f, Vector2.one);
			commandBuffer = new CommandBuffer {name = "ScratchCardRenderer"};
			meshHole = MeshGeneratorTrace.GenerateQuad(Vector3.zero, Vector2.zero);
		}
